...
struct _doubletable 
{     
    char * result;  //Нужен что бы не затерло адрес table_reffer после возврата из потока
    hashtable * table_reffer;
    hashtable * table_url;
};
typedef struct _doubletable doubletable;

enum LOG_ROW {HTTP_TYPE, HTTP_PATH, HTTP_VER, HTTP_CODE, HTTP_BYTES, HTTP_REFFER, LOG_SIZE};
...
int parce_string(const char * line, char ** url, char ** reffer, size_t * datasize){
    //Парсим строку регулярным выдажением
    char * tmp = reg_strstr(line, "\"[A-Z]{3,7}\\s\\S*\\sHTTP\\S*\\s[0-9]*\\s[0-9]*\\s\\S*\\s");
    if(tmp == NULL) {
        return 1;
    }
    int start=0, end=0;
...
void print_statictics(hashtable * table, char * tableinfo){
    ...
    for (size_t j = 0; j < table->size; ++j) {
        if (table->element[j].key != NULL) {
            ...
            if (strcmp(table->element[j].key, EMPTY_REFFER1) == 0 || strcmp(table->element[j].key, EMPTY_REFFER2) == 0) {
                continue;
            }
                ...
                if (url_array[i][0] == '\0'){
                    ...
                    if(count_array[i] < table->element[j].value){
                        for(int k = MAX_ITEM_FORSTAT-1; k>i; --k){
                            strlcpy(url_array[k], url_array[k-1], strlen(url_array[k-1])+1);
...
int main(int argc, char *argv[]){
 ...
 use %ld threads and will use %ld threads!\n", filelen, input_thread_num, filelen);
    ...
    //инициализируем семафор с начальным значением равным максимуму одновременно работающих потоков
