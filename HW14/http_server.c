...
#define REPLY_403 "HTTP/1.0 403 Forbidden\r\n" HEADER_SERVERNAME HEADER_DATE "\r\n"
...
void add_efd_to_epoll(int epollfd, int fd, event_status_t status, uint32_t events)
{
...
void reload_efd_in_epoll(int epollfd, int fd, event_status_t status, uint32_t events)
{
    struct epoll_event ev;
    ev.events = events;
    ev.data.ptr = (void*)pack_event_to_clientfd(fd, status);
...
int handle_request(int client_fd, ssize_t rbytes) {
    ...
    if (length == BUFFER_SIZE) {
        header_bytes = snprintf(client_buffer(client_fd), BUFFER_SIZE, REPLY_413, date);
    ...
    *(client_buffer(client_fd)+length) = '\0';
    if (strcmp((client_buffer(client_fd)+length-4),"\r\n\r\n") == 0){ 
        ...
        if (strncmp(req.method, "GET", 3) != 0) {
            header_bytes = snprintf(client_buffer(client_fd), BUFFER_SIZE, REPLY_405, date);
            printf("Server only support `GET` method with client_fd %d\n", client_fd);
        }
        else if (access(abs_path, F_OK) != 0) {
            header_bytes = snprintf(client_buffer(client_fd), BUFFER_SIZE, REPLY_404, date);
            printf("File not found with client_fd %d\n", client_fd);
        }
        else if (access(abs_path, R_OK) == -1) {
            header_bytes = snprintf(client_buffer(client_fd), BUFFER_SIZE, REPLY_403, date);
            ...
            if(file == NULL){
                ...
                header_bytes = snprintf(client_buffer(client_fd), BUFFER_SIZE, REPLY_200, date, file->size);
...
void read_buffer_from_client(int client_fd, ssize_t * bytes_read) {
    ...
    *bytes_read = read(client_fd, client_buffer(client_fd)+current_length, BUFFER_SIZE-current_length); //client_buffer(client_fd)+current_length
    if ((*bytes_read) < 0) {
...
void send_buffer_to_client(int client_fd){
    ...
    ssize_t current_length = buffer_lengths[client_fd];
    ssize_t writen = write(client_fd, client_buffer(client_fd)+current_length, resp_header_length[client_fd]-current_length);
    if (writen < 0) {
        if (errno == EAGAIN && errno == EWOULDBLOCK)
            return;
        else {
...
void *event_loop(void *args){
    ...
    add_efd_to_epoll(epoll_fd, listen_sock_fd, STATUS_ACCEPT, EPOLLIN | EPOLLEXCLUSIVE);
    ...
    while (1) {
        ...
        for (int i = 0; i < fds_num; i++) {
            ...
            case STATUS_ACCEPT: //generates client_fd
                client_fd = accept4(listen_sock_fd, NULL, NULL, SOCK_CLOEXEC | O_NONBLOCK);
                if (client_fd < 0) {
                    if ((errno == EAGAIN && errno == EWOULDBLOCK)) {
                        break;
                    }
                    ...
                    if ((client_fd - 3 - THREADS_NUM) > MAX_CONNECTIONS) {
                        ...
                        //setup start buffer length & resp_header_length & file_length & file_fds
                        buffer_lengths[client_fd] = 0;
                        ...
                        add_efd_to_epoll(epoll_fd, client_fd, STATUS_READ, EPOLLIN | EPOLLET | EPOLLONESHOT); // 
            ...
            case STATUS_READ: //needed client_fd
                ...
                if (ret) {
                    ...
                    if (result == EXIT_SUCCESS) 
                        reload_efd_in_epoll(epoll_fd, client_fd, STATUS_WRITE, EPOLLOUT | EPOLLET | EPOLLONESHOT);
                    else if (result == EXIT_FAILURE) {
                        reload_efd_in_epoll(epoll_fd, client_fd, STATUS_READ, EPOLLIN | EPOLLET | EPOLLONESHOT);
                        }
                    else {//(result == -1)
            ...
            case STATUS_WRITE: //needed client_fd
                ...
                //Если не отправили все что нужно, перевзводим флаг и заканчиваем итерацию
                if (buffer_lengths[client_fd] < resp_header_length[client_fd]) {
                    reload_efd_in_epoll(epoll_fd, client_fd, STATUS_WRITE, EPOLLOUT | EPOLLET | EPOLLONESHOT);
                }
                else {
...
void setup_server(){
    ...
    setup_buffers(MAX_CONNECTIONS*2+THREADS_NUM+3);  //for max client_id index
