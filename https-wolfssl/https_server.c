...
WOLFSSL_CTX *ctx;
//Массив ssl контекстов
WOLFSSL *sslarr[MAX_CONNECTIONS * 2 + THREADS_NUM + 3];
...
void ssl_read_buffer_from_client(int client_fd, ssize_t *bytes_read) {

  WOLFSSL *ssl = sslarr[client_fd];
  ssize_t current_length = buffer_lengths[client_fd];
  int res = wolfSSL_get_error(ssl, 0); // SSL_FAILURE;
  printf("res %d\n", res);
  // printf("EPOLLIN triggered\n");
  // printf("client IP: %s, port %d\n", inet_ntoa(peer_addr.sin_addr),
  // peer_addr.sin_port);
  *bytes_read = wolfSSL_read(
      ssl, client_buffer(client_fd) + current_length,
...
void ssl_send_buffer_to_client(int client_fd) {

  WOLFSSL *ssl = sslarr[client_fd];
  ssize_t current_length = buffer_lengths[client_fd];
  ssize_t writen =
      wolfSSL_write(ssl, client_buffer(client_fd) + current_length,
                    resp_header_length[client_fd] - current_length);
  if (writen < 0) {
    if (errno == EAGAIN && errno == EWOULDBLOCK)
      return;
    else {
      perror("wolfSSL_write");
      // buffer_lengths[client_fd] = 0;
      return;
    }
  } else if (writen == 0) {
...
void send_file_to_client(int client_fd) {
  ...
  void *file = mmap(NULL, size_send_fd, PROT_READ, MAP_SHARED, send_fd, 0);
  if (file == MAP_FAILED) {
    perror("Error mmap file");
  }
  size_send_fd = wolfSSL_write(sslarr[client_fd], file, size_send_fd);
  if (size_send_fd < 0) {
    if (errno == EAGAIN && errno == EWOULDBLOCK)
      return;
    else {
...
void *event_loop(void *args) {
  ...
  Bind(listen_sock_fd, (struct sockaddr *)&addr, sizeof(addr));
  ...
  while (1) {
    ...
    for (int i = 0; i < fds_num; i++) {
      ...
      case STATUS_ACCEPT: // generates client_fd
        ...
        if (client_fd < 0) {
          ...
          if ((client_fd - 3 - THREADS_NUM) > MAX_CONNECTIONS) {
            ...
            // wolfSSL_negotiate(sslarr[client_fd]);
            sslarr[client_fd] = wolfSSL_new(ctx);

            if (wolfSSL_set_fd(sslarr[client_fd], client_fd) !=
                WOLFSSL_SUCCESS) {
              die("error in setting fd");
            }
      ...
      case STATUS_READ: // needed client_fd
        ...
        ssize_t ret;
        ssl_read_buffer_from_client(client_fd, &ret);
        if (ret > 0) {
          ...
          int err = wolfSSL_get_error(sslarr[client_fd], 0);
          // printf("Error %d\n",err);
          if (err == SOCKET_ERROR_E || err == PARSE_ERROR) {
            wolfSSL_free(sslarr[client_fd]);
            Close(client_fd);
          } else
            reload_efd_in_epoll(epoll_fd, client_fd, STATUS_READ,
                                EPOLLIN | EPOLLET | EPOLLONESHOT);
      ...
      case STATUS_WRITE: // needed client_fd
        ...
        if (buffer_lengths[client_fd] < resp_header_length[client_fd]) {
          ssl_send_buffer_to_client(client_fd);
        }
...
void setup_buffers(int max_connections) {
  ...
  file_length = malloc((max_connections) * sizeof(int));
  if (!file_length)
...
*/
  ...
  if (wolfSSL_CTX_load_verify_buffer(ctx, (const unsigned char *)CA_CRT_PEM,
                                     sizeof(CA_CRT_PEM),
                                     WOLFSSL_FILETYPE_PEM) != 1) {
    die("Error loading CA_CRT_PEM, please check it.\n");
  }

  if (wolfSSL_CTX_use_certificate_buffer(
          ctx, (const unsigned char *)SERV_CRT_PEM, sizeof(SERV_CRT_PEM),
          WOLFSSL_FILETYPE_PEM) != 1) {
    die("Error loading SERV_CRT_PEM, please check it.\n");
  }

  if (wolfSSL_CTX_use_PrivateKey_buffer(
          ctx, (const unsigned char *)SERV_KEY_PEM, sizeof(SERV_KEY_PEM),
          WOLFSSL_FILETYPE_PEM) != 1) {
    die("Error loading SERV_KEY_PEM, please check it.\n");
  }
